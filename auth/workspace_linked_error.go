...
// ChatGPT Team 被停用时，同空间座位会一起失效；只标触发号会让调度继续打兄弟号。
//
// 分组键是请求实际打到的工作区（EffectiveAccountID，含账号级 Chatgpt-Account-Id 覆盖）。
// 兄弟号按原生 AccountID 匹配，避免把「仅请求头覆盖到该空间」的账号整号打死。
...
func (s *Store) MarkDeactivatedWorkspace(trigger *Account, errorMsg string) {
	...
	linkedMsg := fmt.Sprintf("上游返回: deactivated_workspace（工作区联动，由账号 %d 触发）", trigger.DBID)
	for _, acc := range targets {
...
func (s *Store) workspaceLinkedTargets(trigger *Account) []*Account {
	...
	if !s.markWorkspaceLinkedFired(workspaceID) {
		return nil
	}
	var targets []*Account
...
func shouldLinkDeactivatedWorkspace(trigger, sibling *Account, workspaceID string) bool {
	...
	return nativeWorkspaceID(sibling) == workspaceID
}
...
func (s *Store) markWorkspaceLinkedFired(workspaceID string) bool {
	now := time.Now()
	s.workspaceLinkedMu.Lock()
	defer s.workspaceLinkedMu.Unlock()
	if expiry, ok := s.workspaceLinkedRecent[workspaceID]; ok && expiry.After(now) {
		return false
	}
	if s.workspaceLinkedRecent == nil {
		s.workspaceLinkedRecent = make(map[string]time.Time)
	}
	for k, v := range s.workspaceLinkedRecent {
		if !v.After(now) {
			delete(s.workspaceLinkedRecent, k)
		}
	}
	s.workspaceLinkedRecent[workspaceID] = now.Add(workspaceLinkedErrorDedupTTL)
	return true
}
