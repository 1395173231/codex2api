...
| `target_length` | `292` | 非 Team 计划的票据字符长度筛选值 |
| `team_target_length` | `332` | Team/Business 计划的票据字符长度筛选值 |
...
Fernet 的版本和签发时间可以从公开封装读取，但本服务没有签发方密钥，不能验证 HMAC 或解密内容。292/332 是字符长度，不是 HTTP 状态码，也不能证明服务质量或上游认可；默认一小时只是可调整的本地估计。重贴同一票据不会重置签发时间。收到结构合格的 312 字符回带状态时，将其作为重新采集信号，只使本次使用的旧票据失效，不覆盖较新的票据。
