...
采集向 Codex `/responses` 发起最小流式请求，仅接受 HTTP 200 中的 `X-Codex-Turn-State` 响应头或 metadata 的 `current_turn_state`。每次使用全新代理连接，关闭连接复用和 HTTP/2；出口 IP 轮换由代理供应商负责。采集不会使用 Resin 或业务代理，也不会在代理无效时回退直连。探测可能消耗账号上游用量。
