...
## Codex 按模型 Turn-State 票据

管理面板「设置」的 Codex 配置区域提供独立的 Turn-State 采集设置；账号列表、账号详情及编辑窗口按模型展示状态。所有设置直接持久化到数据库，保存后本实例立即生效，其他实例最多约 5 秒同步。默认关闭，必须配置专用采集代理后才可开启。

| 字段 | 默认值 | 含义 |
| --- | --- | --- |
| `enabled` | `false` | 开启后台采集和票据注入 |
| `scheduling_enabled` | `false` | 允许票据接入调度；仅在 `enabled=true` 时生效，配置模型缺少有效票据的 Codex 账号会被选号过滤 |
| `models` | `gpt-6-astra, gpt-5.6-sol` | 1–32 个实际出站模型名称，精确匹配、不支持通配符 |
| `harvest_proxy_url` | 空 | 专用 HTTP(S)/SOCKS5(h) 动态出口代理 |
| `target_lengths` | `[292, 332]` | 可接受的票据字符长度数组；不区分订阅类型，命中任一长度即通过长度检查 |
| `ttl_seconds` | `3600` | 从 Fernet 签发时间起计算的本地有效期，范围 180–3600 秒 |
| `refresh_before_seconds` | `300` | 到期前提前刷新，必须小于 TTL |
| `retry_interval_seconds` | `45` | 失败后的重试间隔，范围 1–3600 秒 |
| `attempt_timeout_seconds` | `25` | 单次探测超时，范围 1–120 秒 |
| `max_attempts` | `3` | 连续失败达到该次数后，退避一个「间隔 × 次数」周期，再继续尝试 |
| `concurrency` | `2` | 每实例同时采集数，范围 1–16；同一账号/模型另有数据库租约去重 |

采集向 Codex `/responses` 发起最小流式请求，仅接受 HTTP 200 中的 `X-Codex-Turn-State` 响应头或 metadata 的 `current_turn_state`。每次使用全新代理连接，关闭连接复用和 HTTP/2；出口 IP 轮换由代理供应商负责。采集不会使用 Resin 或业务代理，也不会在代理无效时回退直连。探测是真实上游请求，会消耗账号请求额度；账号进入积分计费后会消耗积分。

账号处于上游/模型限流、用量窗口耗尽且没有已授权的可用积分、或配额自动暂停时，不会发起新的采集请求；已在途的采集会取消并禁止写回。账号恢复后后台轮询自动继续。若账号已开启积分顶替并且积分余额可用，采集可以继续，探测产生的消耗由该账号积分承担。暂停采集不会删除已有票据，也不会阻止仍有效票据的注入，管理页面会显示 `paused` 以及 `rate_limited` 或 `credits_unavailable` 原因。

业务请求继续使用原来的出口。票据按「账号 ID + 实际出站模型」分别持久化，并绑定账号身份；不同模型或账号之间不做回退。HTTP 注入请求头，WebSocket 同时注入握手头和每次请求帧的 `client_metadata`。没有有效票据时不注入；`scheduling_enabled=false` 时业务仍按原有流程处理，开启后仅对 `models` 中的模型生效，没有对应有效票据的 Codex 账号会在选号阶段跳过，中转与未配置模型不受影响。关闭整体功能后停止采集、注入和调度门控，已存票据仍可查看。旧的全账号共享 `codex_turn_state` 配置不再用于新服务，需在账号窗口按模型重新保存。

Fernet 的版本和签发时间可以从公开封装读取，但本服务没有签发方密钥，不能验证 HMAC 或解密内容。默认配置把 292 和 332 都视为正常票据长度，不区分 Plus、Pro、Team、Business 等订阅类型；这些数字是字符长度，不是 HTTP 状态码，也不能证明服务质量或上游认可。默认一小时只是可调整的本地估计。重贴同一票据不会重置签发时间。每次业务请求都会观测响应头以及 HTTP SSE / WebSocket metadata 帧中的回带状态；只要回带值长度未列入 `target_lengths`，本次请求使用的账号/模型票据会立即停止注入和参与调度，随后以 CAS 方式持久化为失效并触发重新采集。正常长度不会替换当前票据，旧请求也不能撤销并发写入的新票据。

账号页面提供每模型的长度、状态、签发时间、预计到期倒计时、尝试次数、失败原因与下次尝试时间，以及手工保存、删除和立即采集操作。票据原文不在列表/详情接口返回，采集代理认证信息在读回时脱敏；留空或原样保存脱敏值保留已有代理，显式清除需先关闭采集。同现有凭据一样，配置了 `CODEX_CRED_ENCRYPTION_KEY` 时，票据和采集设置会在数据库中加密保存。
