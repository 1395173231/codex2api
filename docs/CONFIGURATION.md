...
业务请求继续使用原来的出口。票据按「账号 ID + 实际出站模型」分别持久化，并绑定账号身份；不同模型或账号之间不做回退。HTTP 注入请求头，WebSocket 同时注入握手头和每次请求帧的 `client_metadata`。没有有效票据时不注入，业务仍按原有流程处理。关闭功能后停止采集与注入，已存票据仍可查看。旧的全账号共享 `codex_turn_state` 配置不再用于新服务，需在账号窗口按模型重新保存。

Fernet 的版本和签发时间可以从公开封装读取，但本服务没有签发方密钥，不能验证 HMAC 或解密内容。默认配置把 292 和 332 都视为正常票据长度，不区分 Plus、Pro、Team、Business 等订阅类型；这些数字是字符长度，不是 HTTP 状态码，也不能证明服务质量或上游认可。默认一小时只是可调整的本地估计。重贴同一票据不会重置签发时间。收到结构合格且未列入 `target_lengths` 的 312 字符回带状态时，将其作为重新采集信号，只使本次使用的旧票据失效，不覆盖较新的票据。
