...
启用后，类别、精确 HTTP 状态码或精确上游错误代码任一命中即可持续重试；`http_4xx`、403、404、上下文错误以及“全部 `response.failed`”等宽泛或确定性故障需要管理员显式选择，501 则已由默认的 `http_5xx` 类别覆盖。默认的 `stream_error` 只匹配首包前的流读取/断开故障，不会把普通 `invalid_request` 类型的 `response.failed` 自动升级为无限重试。

`catch_all` 是默认关闭的超级模式。开启后不再依赖已知类别或错误码清单，任何被代理识别为上游失败的 HTTP 状态、传输错误、流读取错误、`error` 帧或 `response.failed` 都会进入持续换号重试，包括未知状态/代码以及永久额度、余额、鉴权和无效请求错误。管理界面的超级开关会在一次保存中同时设置 `enabled=true` 和 `catch_all=true`；关闭总开关会同步清除 `catch_all`，避免隐藏启用。该模式可能重复消耗 input/output token、请求额度和账号配额，并让客户端无限等待，必须显式承担风险后开启。

结构化安全策略拒绝始终立即停止，不能被超级模式、通用类别或精确选择器覆盖。持续模式只覆盖首包前且尚未向下游写正文的可安全重放请求；客户端取消、下游写入失败或 WebSocket 断开会结束等待。图片请求保留 5 次总尝试硬上限，Grok 图片/视频媒体请求保留 3 次总尝试硬上限，避免昂贵或可能产生副作用的媒体操作无限提交。
