...
`catch_all` 是默认关闭的超级模式。开启后不再依赖已知类别或错误码清单，任何真实上游 HTTP、传输、流读取、`error`、`response.failed` 或未知失败都会进入持续重试，包括永久额度、余额、鉴权、无效请求和结构化安全策略错误。文本推理只接受上游 HTTP `200` 及协议正常终态；其他状态、失败终态及无终态 EOF 都会丢弃整次尝试并继续。管理界面的超级开关会在一次保存中同时设置 `enabled=true` 和 `catch_all=true`；关闭总开关会同步清除 `catch_all`，避免隐藏启用。

持续重试会把每次流式上游尝试完整暂存；失败整次丢弃，正常终态才一次性回放。因此客户端等待时由 SSE 注释或 Responses WebSocket Ping 保活，但不再实时逐 token 收到生成结果。普通自选模式不会无限重试未选中的结构化安全策略拒绝；只有 `catch_all` 覆盖该保护。非流式 JSON 无法插入应用层心跳，仍受中间代理 idle timeout 约束。
...
超级模式遇到结构化 `cyber_policy` 时仍记录本地审计与 signed decision，但不由该中间失败创建本地 conversation lock；否则透明重试虽然最终成功，下一轮任务仍会被旧锁阻断。此行为是超级模式明确接受的安全取舍，普通自选模式不改变原有锁定逻辑。
