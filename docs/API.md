...
`continuous_retry_catch_all` 是默认关闭的超级开关。请求同时提交 `continuous_retry_enabled=true` 与 `continuous_retry_catch_all=true` 后，所有被代理识别为上游失败的 HTTP 状态、传输/读取失败、`error` 帧和 `response.failed` 都会进入持续换号重试，不依赖当前已知错误码清单；永久额度、余额、鉴权、无效请求和以后新增的未知错误也包含在内。超级模式可能无限重复消耗 input/output token、请求额度与账号配额，并让客户端长期等待。`continuous_retry_enabled=false` 会规范化为 `continuous_retry_catch_all=false`。

`retry_interval_ms` 是本地等待下限；有效的上游 `Retry-After` 优先并限制在 5 分钟内。持续模式没有有效 `Retry-After` 时还会使用带抖动的指数退避，从 250ms 起步并在 30 秒封顶。结构化安全策略拒绝不会被超级模式覆盖。流已经向下游写出内容后不会整段重放，以免重复文本或工具副作用。图片请求仍受 5 次总尝试上限约束，Grok 图片/视频媒体请求仍受 3 次总尝试上限约束，持续模式不会移除这些昂贵或带副作用操作的硬上限。客户端取消、下游写入失败或 WebSocket 已断开时等待会立即停止。
