...
func New(driver string, dsn string, schema ...string) (*DB, error) {
	...
	if !fromSchemaTemplate {
		...
		if err := db.ensureCodexTurnStateSchema(ctx); err != nil {
			return nil, fmt.Errorf("initialize Codex turn-state tables: %w", err)
		}
...
func (db *DB) migrate(ctx context.Context) error {
	...
	ALTER TABLE usage_logs ADD COLUMN IF NOT EXISTS ultra BOOLEAN DEFAULT false;
	...
	ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS auto_reset_credits_on_limit_enabled BOOLEAN DEFAULT FALSE;
	ALTER TABLE system_settings ADD COLUMN IF NOT EXISTS codex_analytics_enabled BOOLEAN DEFAULT FALSE;
...
type SystemSettings struct {
	...
	// AutoResetCreditsOnLimitEnabled 控制 Plus/Pro 主额度耗尽时自动使用重置卡（默认关闭）。
	AutoResetCreditsOnLimitEnabled bool
	CodexAnalyticsEnabled          bool // 向 OpenAI 上报最小 Codex 事件统计，默认关闭
	...
	err := db.conn.QueryRowContext(ctx, `
		SELECT COALESCE(site_name, 'CodexProxy'), COALESCE(site_logo, ''),
		       ...
		       COALESCE(public_account_portal_page_enabled, false),
			       ...
			       COALESCE(auto_reset_credits_on_limit_enabled, false),
			       COALESCE(codex_analytics_enabled, false),
		...
		&s.AutoResetCreditsOnLimitEnabled,
		&s.CodexAnalyticsEnabled,
	...
	_, err := db.conn.ExecContext(ctx, `
			INSERT INTO system_settings (
				...
				public_image_studio_page_enabled,
					...
					auto_reset_credits_on_limit_enabled,
			...
			auto_clean_rate_limited = EXCLUDED.auto_clean_rate_limited,
				...
				public_image_studio_page_enabled = EXCLUDED.public_image_studio_page_enabled,
					...
					ignore_usage_limit_status = EXCLUDED.ignore_usage_limit_status,
						...
						auto_reset_credits_on_limit_enabled = EXCLUDED.auto_reset_credits_on_limit_enabled,
						codex_analytics_enabled = EXCLUDED.codex_analytics_enabled,
		...
		s.IgnoreUsageLimitStatus, s.AutoResetCreditsEnabled, s.AutoResetCreditsOnLimitEnabled, s.CodexAnalyticsEnabled,
