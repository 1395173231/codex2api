...
export function mergeAccountLiveState<T extends { id: number; active_requests?: number; occupied_requests?: number }>(
  current: T[],
  ...
  const next = current.map((account) => {
    ...
    changed = true
    return { ...account, active_requests: activeRequests, occupied_requests: occupiedRequests }
  })
