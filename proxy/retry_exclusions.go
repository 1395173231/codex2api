...
func (h *Handler) waitForRetryAccountAvailable(ctx context.Context, affinityKey string, apiKeyID int64, exclude map[int64]bool, filter auth.AccountFilter, preserveBinding bool, policy auth.DispatchPolicy) (*auth.Account, string, error) {
	account, proxyURL, _, err := h.waitForRetryAccountAvailableWithGuard(ctx, affinityKey, apiKeyID, nil, exclude, filter, preserveBinding, policy)
	return account, proxyURL, err
}

func (h *Handler) waitForRetryAccountAvailableWithGuard(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclude map[int64]bool, filter auth.AccountFilter, preserveBinding bool, policy auth.DispatchPolicy) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	if ctx == nil {
	...
	account, proxyURL, guard, err := h.store.WaitForDispatchAvailableWithMessageHashes(ctx, affinityKey, 30*time.Second, apiKeyID, messageHashes, exclude, filter, preserveBinding, policy, heartbeat)
...
func (h *Handler) nextRetryAccountForSession(ctx context.Context, affinityKey string, apiKeyID int64, exclusions *retryAccountExclusions, filter auth.AccountFilter) (*auth.Account, string) {
	return h.nextRetryAccount(ctx, affinityKey, apiKeyID, nil, exclusions, filter, false, auth.DispatchPolicyStandard)
}

func (h *Handler) nextRetryAccountForSessionWithMessageHashes(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter) (*auth.Account, string) {
	return h.nextRetryAccount(ctx, affinityKey, apiKeyID, messageHashes, exclusions, filter, false, auth.DispatchPolicyStandard)
}

func (h *Handler) nextRetryAccountForSessionWithDispatch(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter, policy auth.DispatchPolicy) (*auth.Account, string) {
	return h.nextRetryAccount(ctx, affinityKey, apiKeyID, messageHashes, exclusions, filter, false, policy)
}
...
func (h *Handler) nextRetryAccountForSessionWithDispatchGuard(ctx context.Context, affinityKey string, apiKeyID int64, exclusions *retryAccountExclusions, filter auth.AccountFilter, policy auth.DispatchPolicy) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	return h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, nil, exclusions, filter, false, policy)
}

func (h *Handler) nextRetryAccountForSessionWithMessageHashesAndDispatchGuard(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter, policy auth.DispatchPolicy) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	return h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, messageHashes, exclusions, filter, false, policy)
}
...
func (h *Handler) nextRetryAccountForSessionWithGuard(ctx context.Context, affinityKey string, apiKeyID int64, exclusions *retryAccountExclusions, filter auth.AccountFilter) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	return h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, nil, exclusions, filter, false, auth.DispatchPolicyStandard)
}

func (h *Handler) nextRetryAccountForSessionWithMessageHashesAndGuard(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	return h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, messageHashes, exclusions, filter, false, auth.DispatchPolicyStandard)
}
...
func (h *Handler) nextRetryAccountForContinuation(ctx context.Context, affinityKey string, apiKeyID int64, exclusions *retryAccountExclusions, filter auth.AccountFilter) (*auth.Account, string) {
	return h.nextRetryAccount(ctx, affinityKey, apiKeyID, nil, exclusions, filter, true, auth.DispatchPolicyStandard)
}
...
func (h *Handler) nextRetryAccountForContinuationWithDispatch(ctx context.Context, affinityKey string, apiKeyID int64, exclusions *retryAccountExclusions, filter auth.AccountFilter, policy auth.DispatchPolicy) (*auth.Account, string, error) {
	account, proxyURL, _, err := h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, nil, exclusions, filter, true, policy)
	return account, proxyURL, err
}

func (h *Handler) nextRetryAccount(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter, preserveBinding bool, policy auth.DispatchPolicy) (*auth.Account, string) {
	account, proxyURL, _, _ := h.nextRetryAccountWithGuard(ctx, affinityKey, apiKeyID, messageHashes, exclusions, filter, preserveBinding, policy)
	return account, proxyURL
}

func (h *Handler) nextRetryAccountWithGuard(ctx context.Context, affinityKey string, apiKeyID int64, messageHashes []uint64, exclusions *retryAccountExclusions, filter auth.AccountFilter, preserveBinding bool, policy auth.DispatchPolicy) (*auth.Account, string, auth.SessionAffinityGuard, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	for {
		...
		if preserveBinding {
			...
			account, stickyProxyURL, guard = h.nextAccountForSessionWithMessageHashesWithDispatchGuard(affinityKey, apiKeyID, messageHashes, exclude, filter, policy)
		}
		if account != nil {
			if ctx.Err() != nil {
		...
		account, stickyProxyURL, guard, admissionErr = h.waitForRetryAccountAvailableWithGuard(ctx, affinityKey, apiKeyID, messageHashes, exclude, filter, preserveBinding, policy)
		if account != nil {
			if ctx.Err() != nil {
				h.store.Release(account)
				return nil, "", auth.SessionAffinityGuard{}, nil
