...
func TestSchedulerQueueOverloadStopsContinuousRetry(t *testing.T) {
	...
	defer cancel()
	_, _, _, err := h.nextRetryAccountWithGuard(ctx, "", 0, nil, exclusions, nil, false, auth.DispatchPolicyStandard)
...
func TestSchedulerWaitHeartbeatPreservesOneAdmission(t *testing.T) {
	...
	defer cancel()
	_, _, _, _ = h.waitForRetryAccountAvailableWithGuard(ctx, "", 0, nil, nil, nil, false, auth.DispatchPolicyStandard)
