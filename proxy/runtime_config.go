...
type RuntimeSettings struct {
	...
	// AutoResetCreditsOnLimitEnabled 控制 Plus/Pro 主额度耗尽时自动使用重置卡（默认 false）。
...
func ApplyRuntimeSettingsFromSystem(settings *database.SystemSettings) RuntimeSettings {
	...
	if settings != nil {
		...
		next.AutoResetCreditsOnLimitEnabled = settings.AutoResetCreditsOnLimitEnabled
		next.CodexAnalyticsEnabled = settings.CodexAnalyticsEnabled
